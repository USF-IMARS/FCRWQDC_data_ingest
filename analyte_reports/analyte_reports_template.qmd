...
format: html
params:
  analyte: "Salinity"  # must be "Salinity" before `quarto publish`
---
...
full_df <- read.csv(here("data", "exports", "dashboardData.csv"))
...
```{R, Show units used by each program}
#| code-summary: Show units used by each program
library(ggplot2)
...
```{R, Show values grouped by units}
#| code-summary: Show values grouped by units
library(ggplot2)
...
```{R, Show value distributions for each program}
#| code-summary: Show value distributions for each program
library(ggplot2)
