...
df %>%
  mutate(
    source = DEP.Program.Name,
    site = DEP.Monitoring.Location.ID,
    datetime = DEP.Activity.Start.Date.Time,
    value = DEP.Result.Value.Number,
    units = DEP.Result.Unit,
    latitude = DEP.Monitoring.Location.Latitude,
    longitude = DEP.Monitoring.Location.Longitude,
    sample_depth = DEP.Monitoring.Location.SampleDepth,
    .keep = "none"
) %>%
...
samples_df <- df %>%
  select(
    ...
    DEP.Monitoring.Location.Latitude,
    DEP.Monitoring.Location.Longitude,
    DEP.Monitoring.Location.SampleDepth
    ) %>%
...
sample_stats_df <- samples_df %>%
  ...
  reframe(
    n_values = n(),
    # add slope, p.value, tau
    !!! seasonalMannKendallVectorized(
      Activity.Start.Date.Time,
...
tryCatch({
  ...
  print("Error in p.value color column")
  print(e)
})
