---
title: "BROWARD Data"
format: html
params: 
  org_id: BROWARD #FIU_WQMP #AOML_BBWW #DERM_BBWQ  # must be BROWARD before rendering
---

```{R evaluate dataset completeness}
# read data from source files (takes a long time)
source(here("R/getData.R"))
df <- getData(params$org_id)
source(here("R/mutateWINToSTORET.R"))
df <- df %>% mutateWINToSTORET(.keep = "unused")
# your full list
STORET_COLUMNS <- c(
  "ExportVersion","RowID","ProgramID","ProgramName","Habitat",
  "IndicatorID","IndicatorName","ParameterID","ParameterName",
  "ParameterUnits","ProgramLocationID","AreaID","ManagedAreaName",
  "Region","ActivityType","SampleDate","ResultValue","Year","Month",
  "ActivityDepth_m","RelativeDepth","TotalDepth_m","MDL","PQL",
  "DetectionUnit","ValueQualifier","ValueQualifierSource",
  "SampleFraction","ResultComments","OriginalLatitude",
  "OriginalLongitude","SEACAR_QAQCFlagCode",
  "SEACAR_QAQC_Description","Include","SEACAR_EventID","MADup"
)

# figure out which are actually present
present_cols <- intersect(STORET_COLUMNS, names(df))
missing_cols <- setdiff(STORET_COLUMNS, names(df))

if (length(missing_cols) > 0) {
  warning("\nThese STORET_COLUMNS were not found in df:\n  ",
          paste(missing_cols, collapse = ", "))
}

# compute percent‐complete only over the present ones
completeness <- df %>%
  summarise(
    across(
      any_of(present_cols),
      ~ mean(!is.na(.)) * 100,
      .names = "pct_complete_{col}"
    )
  ) %>%
  pivot_longer(
    cols      = everything(),
    names_to  = "column",
    values_to = "pct_complete"
  ) %>%
  mutate(column = sub("^pct_complete_", "", column))

print(completeness)

# 2) Any additional columns in df that are not mapped to STORET
extra_columns <- setdiff(names(df), STORET_COLUMNS)
cat("\nColumns from source data that are not mapped to STORET columns:\n")
print(extra_columns)
```

Provider Data Assessment

```{R setup}
librarian::shelf(
  dplyr,
  glue,
  here,
  tidyr
)
```

```{R getData}
#| code-summary: read in data

# read from cached file produced by index.qmd
df_allData <- read.csv(here("data", "exports", "allData.csv")) %>%
  filter(source == params$org_id)
df <- df_allData



df$value <- as.numeric(df$value)
df$datetime <- as.POSIXct(df$datetime, format = "%Y-%m-%d")
```

```{R create violin plot}
# Load ggplot2
library(ggplot2)

ggplot(df, aes(x = analyte, y = value)) +
  geom_violin(width = 1.2, adjust = 10, fill = "skyblue", color = "black") +
  geom_boxplot(width = 0.1, outlier.shape = NA) +
  scale_y_log10() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Analyte", y = "Log10(Value)", title = "Violin + Boxplot")

```

```{R create presence table}
library(dplyr)
library(tidyr)
library(ggplot2)

source(here::here("R/presenceTable.R"))

heat_long <- getPresenceHeatmapTable(df)
```

```{R analyte presence table, fig.width=10, fig.height=min(max(6, length(unique(heat_long$site)) * 0.2),20)}
plotHeatmap(heat_long)
```


```{R create map }
#| code-summary: create map with unique points

# check for missing lat/lon
if (all(is.na(df$latitude)) || all(is.na(df$longitude))) {
  cat("Cannot make map: All rows have missing latitude or longitude.")
} else {
  library(leaflet)
  library(dplyr)

  # 1. Filter out rows & keep only one per unique lat/lon
  df_unique_locs <- df %>%
    filter(!is.na(latitude), !is.na(longitude)) %>%
    distinct(latitude, longitude, .keep_all = TRUE)

  # 2. Plot all unique points (no clustering)
  leaflet(df_unique_locs) %>%
    addProviderTiles(providers$OpenStreetMap) %>%
    addCircleMarkers(
      lng         = ~longitude,
      lat         = ~latitude,
      radius      = 4,
      color       = "blue",
      fillOpacity = 0.5,
      popup       = ~paste0(
        "Monitoring Location: ", site
      )
      # clusterOptions removed to show every point at all zoom levels
    )
}
```

```{R prep data through time}
#| code-summary: visualize distribution of points through time
library(ggplot2)
library(lubridate)

# Create a dataframe with unique sampling events
sampling_events <- df %>%
  distinct(datetime, site) %>%
  arrange(datetime)
```

```{R time distribution plot}
#| code-summary: Create time distribution plot
ggplot(sampling_events, aes(x = datetime)) +
  geom_histogram(bins = 30, fill = "steelblue", color = "white", alpha = 0.7) +
  labs(
    title = "Distribution of Sampling Events Over Time",
    x = "Date",
    y = "Number of Sampling Events"
  ) +
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    panel.grid.minor = element_blank()
  )

```

```{R sampling heatmap, fig.width=10, fig.height=min(max(6, length(unique(heat_long$site)) * 0.2),20)}
# Create a heatmap of sampling locations over time
# drop rows with missing dates or IDs
sampling_events <- sampling_events %>%
  filter(!is.na(datetime), !is.na(site))
# if sampling_events is empty, skip this plot
if (nrow(sampling_events) > 0) {
  ggplot(sampling_events, 
          aes(x = datetime, 
              y = site)) +
      geom_point(color = "steelblue", alpha = 0.7) +
      labs(
        title = "Sampling Locations Over Time",
        x = "Date",
        y = "site"
      ) +
      theme_minimal() +
      theme(
        axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.minor = element_blank()
      )
} else {
  print("No sampling events with valid dates & IDs")
}
```

```{R print start + end dates}
date_range <- range(df$`datetime`, na.rm = TRUE)
  cat("Earliest sampling date: ", format(date_range[1], "%Y-%m-%d %H:%M:%S"), "\n")
  cat("Latest   sampling date: ", format(date_range[2], "%Y-%m-%d %H:%M:%S"), "\n")
```


