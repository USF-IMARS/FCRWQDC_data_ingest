...
# analyte_columns is your vector of column names
# analyte_columns <- c("A", "B", "C", ...)

# Step 1: convert selected columns to logical presence (TRUE = valid numeric)
presence <- df_wide %>%
  mutate(across(all_of(analyte_columns),
                ~ !is.na(suppressWarnings(as.numeric(.))))) %>%
  select(all_of(analyte_columns))
...
# Convert co-occurrence matrix to tidy format
...
ggplot(cooccurrence_df, aes(x = Analyte1, y = Analyte2, fill = Cooccurrence)) +
  ...
  scale_fill_gradientn(
    colors = rainbow(7)  # rainbow with n colors, low -> high
  ) +
  labs(title = "Analyte Co-occurrence Matrix",
