...
# 4. tidy it and set factor levels explicitly so ggplot uses that order
cooccurrence_df <- as.data.frame(cooccurrence_matrix) %>%
  ...
  mutate(
    ...
    # if you want the same visual order top->bottom as left->right, use analyte_order:
    Analyte2 = factor(Analyte2, levels = analyte_order)
    # if you'd prefer the top row to show the most frequent analyte (mirror x),
    # uncomment the next line instead:
    # Analyte2 = factor(Analyte2, levels = rev(analyte_order))
  )
...
ggplot(cooccurrence_df, aes(x = Analyte1, y = Analyte2, fill = Cooccurrence)) +
  ...
  labs(
    title = "Analyte Co-occurrence Matrix (Sorted by Observation Count)",
    x = NULL,
    y = NULL,
    fill = "Co-occurrence"
  ) +
